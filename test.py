...
class InsertData(QDialog):
    ...
    def addmember(self):#function
        ...
        with conn:
            try:
                cur.execute(f'insert into Product(Product_Name,Product_Description,Classification,Quantity,Price) \
                   values("{name}","{descr}","{clas}",{Qty},{price})')
                conn.commit()
